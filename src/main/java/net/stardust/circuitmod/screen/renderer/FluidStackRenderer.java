...
public class FluidStackRenderer {
     ...
     */
    public void drawFluid(DrawContext context, int fluidLevel, int x, int y, int width, int height, long maxCapacity, FluidVariant fluidVariant) {
        ...
        final int drawHeight = (int) Math.ceil((fluidLevel / (float) maxCapacity) * height);
    ...
    public List<Text> getTooltip(SingleVariantStorage<FluidVariant> fluidStorage, TooltipContext tooltipFlag) {
        ...
        if (tooltipMode == TooltipMode.SHOW_AMOUNT_AND_CAPACITY) {
            MutableText amountString = Text.translatable("spacemod.tooltip.liquid.amount.with.capacity", nf.format(amount), nf.format(capacityMb));
            tooltip.add(amountString.fillStyle(Style.EMPTY.withColor(Formatting.DARK_GRAY)));
        } else if (tooltipMode == TooltipMode.SHOW_AMOUNT) {
            MutableText amountString = Text.translatable("spacemod.tooltip.liquid.amount", nf.format(amount));
            tooltip.add(amountString.fillStyle(Style.EMPTY.withColor(Formatting.DARK_GRAY)));
        }
