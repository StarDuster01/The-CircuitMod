...
public class CrusherInventoryOutSlaveBlock extends BlockWithEntity {
    public CrusherInventoryOutSlaveBlock(Settings settings) {
        super(settings);
    }
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new CrusherInventoryOutSlaveBlockEntity(pos, state);
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, ModBlockEntities.CRUSHER_INVENTORY_OUT_SLAVE_BE, (world1, pos, state1, blockEntity) -> blockEntity.tick(world1,pos,state1));
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!world.isClient) {
            ...
            if (be instanceof CrusherInventoryOutSlaveBlockEntity) {
                BlockPos masterPos = ((CrusherInventoryOutSlaveBlockEntity) be).getMasterPos();
